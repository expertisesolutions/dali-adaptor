...
struct TriggerEvent::Impl final
{
  ...
  {
    const auto myId = nameId.fetch_add(1, std::memory_order_relaxed);

    std::stringstream ss(EventName);
    ss << myId;
    name = [NSString stringWithUTF8String:ss.str().c_str()];
...
private:
  ...
  static std::atomic_uint64_t nameId;
};

std::atomic<uint64_t> TriggerEvent::Impl::nameId{0};
...
{
  auto *notification = [NSNotification notificationWithName:mImpl->name object:nil];
  ...
  [queue enqueueNotification:notification postingStyle:NSPostASAP coalesceMask:0 forModes:nil];
}
...
-(NotificationObserver *) initTriggerImpl:(Dali::Internal::Adaptor::TriggerEvent::Impl *) impl;
{
  ...
  if (self)
  {
    mImpl = impl;
    auto *center = [NSNotificationCenter defaultCenter];
    [center addObserver:self selector:@selector(ReceiveNotification) name:mImpl->name object:nil];
  }
